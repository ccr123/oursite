...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'clear ' . $class ); ?>>
        ...
        <div class="entry-content">
            <?php
				the_content( sprintf(
					...
					wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'corponess' ), array( 'span' => array( 'class' => array() ) ) ),
					the_title( '<span class="screen-reader-text">"', '"</span>', false )
				) );

				wp_link_pages( array(
					'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'corponess' ),
					'after'  => '</div>',
				) );
			?>
        </div><!-- .entry-content -->
   ...
   <div class="entry-meta">
        <?php
        	corponess_single_categories();
        	corponess_entry_footer();
    	?>
    </div><!-- .entry-meta -->
