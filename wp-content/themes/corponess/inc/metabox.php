...
class Corponess_MetaBox {
    ...
    */
    public function show() {
        ...
        <div id="corponess-ui-tabs" class="ui-tabs">
            <ul class="corponess-ui-tabs-nav" id="corponess-ui-tabs-nav">
                <li><a href="#frag1"><?php esc_html_e( 'Layout Options', 'corponess' ); ?></a></li>
                <li><a href="#frag2"><?php esc_html_e( 'Select Sidebar', 'corponess' ); ?></a></li>
            </ul> 
            ...
            <div id="frag1" class="catch_ad_tabhead">
                <table id="layout-options" class="form-table" width="100%">
                    <tbody>
                        <tr>
                            <?php  
                                $metalayout = get_post_meta( $post->ID, 'corponess-sidebar-position', true );
                                if( empty( $metalayout ) ){
                                    $metalayout='';
                                }
                                ...
                                foreach ( $layout_options as $value => $url ) :
                                    echo '<label>';
                                    echo '<input type="radio" name="corponess-sidebar-position" value="' . esc_attr( $value ) . '" ' . checked( $metalayout, $value, false ) . ' />';
            ...
            <div id="frag2" class="catch_ad_tabhead">
                <table id="sidebar-metabox" class="form-table" width="100%">
                    <tbody> 
                        <tr>
                            <?php
                            $metasidebar = get_post_meta( $post->ID, 'corponess-selected-sidebar', true );
                            ...
                            ?>
                                <td style="vertical-align: top;">
                                    <label class="description">
                                        <input type="radio" name="corponess-selected-sidebar" value="<?php echo esc_attr( $field ); ?>" <?php checked( $field, $metasidebar ); ?>/>&nbsp;&nbsp;<?php echo esc_html( $value ); ?>
                                    </label>
                                </td>
    ...
    public function save( $post_id ) { 
        ...
        $is_valid_nonce = ( isset( $_POST[ 'corponess_nonce' ] ) && wp_verify_nonce( sanitize_key( $_POST[ 'corponess_nonce' ] ), basename( __FILE__ ) ) ) ? 'true' : 'false';
